if SERVER then
  resource.AddFile("materials/vgui/ttt/icon_rg_defibrillator_traitor.png")
end
...
if CLIENT then
  SWEP.PrintName = "Traitor Defibrillator"
  SWEP.Slot = 7

  SWEP.Icon = "vgui/ttt/icon_rg_defibrillator_traitor.png"

  SWEP.EquipMenuData = {
    type = "item_weapon",
    name = "Traitor Defibrillator",
    desc = "Resurrect dead corpses! Revived people\nwill be traitor regardless of past role.\nRevived player will have 50 HP.\n\nOrginnally by: Willox\nEdited by: Toxic_Terrorists"
  }
...
function SWEP:BeginDefib(ply, ragdoll)
  ...
  self:SetStateText("DEFIBRILLATING AS TRAITOR - "..string.upper(ply:Name()))
...
function SWEP:HandleRespawn()
  ...
  ply:SetEyeAngles(Angle(0, ragdoll:GetAngles().y, 0))
	...
	for k, v in pairs(player.GetAll()) do
		v:ChatPrint(ply:Nick() ..  " has been brought back to life as a Traitor!" )
	end
